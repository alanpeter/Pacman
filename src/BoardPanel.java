...
public class BoardPanel extends JPanel implements ActionListener {
   ...
   public void actionPerformed(ActionEvent e)
   {
	   ...
	   else if (((JButton)e.getSource()).getText().compareTo("dropFood") == 0) {
           player.setDropFood(true);
       }
	}
...
//        if(player.isDropFood()){
            cell = player.getFoodCell();
            gr.setColor(Color.YELLOW);
            gr.fillOval(xCor(cell.col)+CELLWIDTH/8, yCor(cell.row)+CELLWIDTH/8, CELLWIDTH*3/4, CELLHEIGHT*3/4);
            gr.setColor(Color.WHITE);
            gr.drawString("F",xCor(cell.col)+CELLWIDTH/3, yCor(cell.row)+2*CELLWIDTH/3);
            player.setDropFood(false);
//        }
    }
}